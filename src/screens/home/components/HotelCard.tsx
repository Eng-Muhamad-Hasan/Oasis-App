...
export function HotelCard({
  ...
  return (
    <PressableScale scaleTo={0.97} className="w-52" onPress={onPress}>
        ...
        >
          <Animated.View style={heartStyle}>
            <Ionicons
              ...
              size={17}
              color={favorite ? colors.dark.secondary : colors.dark.ink}
            />
          </Animated.View>
        </Pressable>
      </View>
      ...
      <Text
        className="mt-2.5 font-jakarta-semibold text-[15px] text-ink"
        numberOfLines={1}
      >
        {hotel.name}
      </Text>
      <View className="mt-1 flex-row items-center gap-1">
        ...
        <Text className="font-jakarta-medium text-[13px] text-ink">
          {hotel.rating}{" "}
          <Text className="font-jakarta text-al-teal-100">
            ({hotel.reviews} reviews)
          </Text>
        </Text>
      </View>
      <Text className="mt-1 font-jakarta text-[12px] text-al-teal-100">
        {hotel.city} · {hotel.distance}
      </Text>
      <View className="mt-2 flex-row gap-1.5">
        {hotel.perks.map((perk) => (
          <View key={perk} className="rounded-lg bg-chip px-2 py-1">
            <Text className="font-jakarta-medium text-[10px] text-ink/80">
              {perk}
            </Text>
          </View>
        ))}
