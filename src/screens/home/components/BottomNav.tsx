...
const TAB_ICONS: Record<BottomTab, any> = {
  [BottomTab.Home]: 'home',
...
export function BottomNav({ active, onChange }: BottomNavProps) {
  ...
  return (
    ...
    >
      <View
        className="h-15.5 flex-1 flex-row items-center rounded-full bg-pill px-1.5"
        ...
        {Object.values(BottomTab).map((tab) => {
          if (tab === active) {
            return (
              <View
                key={tab}
                className="h-12.5 flex-row items-center gap-2 rounded-full bg-white px-5"
              >
          ...
          return (
            <Pressable
              key={tab}
              className="h-12.5 flex-1 items-center justify-center"
              onPress={() => {
      ...
      <PressableScale
        scaleTo={0.92}
        className="ml-3 h-15.5 w-15.5 items-center justify-center rounded-full bg-pill"
