...
export const OtpInput: FC<IOtpInput> & FunctionComponent<IOtpInput> =
  memo<IOtpInput>(
    ({
      ...
      const [otpValue, setOtpValue] = useState<string[]>(initialValue);

      useEffect(() => {
        if (typeof value === "string") {
          setOtpValue(
            Array.from({ length: otpCount }, (_, index) => value[index] ?? ""),
          );
        }
      }, [value, otpCount]);
      ...
      const commitOtpValue = (nextValue: string[]) => {
      ...
      const onPress = () => {
        if (focus === -1) {
          ...
          nextValue[otpCount - 1] = "";
          commitOtpValue(nextValue);
          inputRef.current[otpCount - 1].current.focus();
        } else {
      ...
      useEffect(() => {
        ...
        onInputChange?.(code);
        if (code.length === otpCount && onInputFinished) {
          if (!error) {
