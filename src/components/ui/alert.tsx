...
export function Alert({ title, body, tone = 'info', icon }: AlertProps) {
  ...
  const toneStyles = {
    ...
    success: { backgroundColor: colors.successSoft, borderColor: colors.success, icon: 'checkCircle' },
    warning: { backgroundColor: colors.warningSoft, borderColor: colors.warning, icon: 'alert' },
    danger: { backgroundColor: colors.dangerSoft, borderColor: colors.danger, icon: 'error' },
  } as const;
